    ...
    if (!this.fileProgressWrapper.length) {
       ...
       // console.log(Wrappeer.toString());
        this.fileProgressWrapper=Wrappeer=['<li id="a'+this.fileProgressID+'" data-size="'+file.size+'" class="clearfix progressContainer">',
            ...
            '            <span><img src="" id="imgUrl"><a href="javascript:" class="close"></a></span>',
            '        </div>',
            '        <div class="inner-bar" style="display:block;">',
            '            <div class="amount">',
            '                冲洗数量&emsp;',
            '                <a href="javascript:" class="btn-minus"></a><span class="show-count">1</span>',
            '                <a href="javascript:" class="btn-plus"></a>&emsp;￥<span class="Unit-Price">0.80</span> /张<span class="hide total-single">0.00</span>',
            '                <!--单个商品总价-->',
            '            </div>',
            ...
            '                <dd data-value="6.5">6.5寸</dd>',
            '                <dd data-value="7">7寸</dd>',
            '                <dd data-value="3">3寸</dd>',
            ...
            '</li>'].join("");
            if(w.length>0){
                w.before(Wrappeer);//添加
            }else{
                window.removeItem();//删除上面的节点
...
FileProgress.prototype.setComplete = function(up, info,file) {
    ...
    //设置url
    var td_1=this.fileProgressWrapper.find('#imgUrl');
    ...
    if (!isImg) {
        ...
        td_1.attr("src",'images/default.png');
        ...
        //$(img).attr('src', url);
        td_1.attr("src",url);//展示缩略图
        this.setTop(82);//重置上传按钮位置

        var that=this;
        //图片下载完成后，重置上传按钮
        td_1.on('load',function () {
            that.setTop(0);
        });
...
FileProgress.prototype.setTop = function (num) {
